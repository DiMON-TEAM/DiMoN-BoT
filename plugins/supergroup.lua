...
local function lock_group_forward(msg, data, target)
  ...
  local group_forward_lock = data[tostring(target)]['settings']['forward']
  if group_forward_lock == 'yes' then
    return 'forward is already locked'
  else
    data[tostring(target)]['settings']['forward'] = 'yes'
    save_data(_config.moderation.data, data)
...
local function unlock_group_forward(msg, data, target)
  ...
  if group_forward_lock == 'no' then
    ...
    data[tostring(target)]['settings']['forward'] = 'no'
    save_data(_config.moderation.data, data)
		...
		unlock_group_operator(msg, data, target),
      	...
      	return lock_group_etehad(msg, data, target), etehad
			...
			if matches[2] == 'forward' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] locked forward")
