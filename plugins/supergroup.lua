...
local function check_member_super(cb_extra, success, result)
  ...
  if success == 0 then
	send_large_msg(receiver, "اول من را ادمین کنید ")
  end
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
	  local text = '!سوپر گروه با موفقیت افزوده شد'
...
local function check_member_superrem(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
	  local text = '!سوپر گروه با موفقیت ریمو شد'
      return reply_msg(msg.id, text, ok_cb, false)
    end
  end
end
...
local function lock_group_etehad(msg, data, target)
  ...
  if group_etehad_lock == 'yes' then
    return ' 🔒مدل اتحاد فعال بود🔒 '
  else
    data[tostring(target)]['settings']['etehad'] = 'yes'
    save_data(_config.moderation.data, data)
    return ' 🔒مدل اتحاد فعال شد🔒 '
  end
end
...
local function unlock_group_etehad(msg, data, target)
  ...
  if group_etehad_lock == 'no' then
    return ' 🔓مدل اتحاد فعال نیست🔓 '
  else
    data[tostring(target)]['settings']['etehad'] = 'no'
    save_data(_config.moderation.data, data)
    return ' 🔓مدل اتحاد غیر فعال شد🔓 '
  end
end
...
local function lock_group_leave(msg, data, target)
  ...
  if group_leave_lock == 'yes' then
    return ' 🔒قفل خروج فعال بود🔒 '
  else
    data[tostring(target)]['settings']['leave'] = 'yes'
    save_data(_config.moderation.data, data)
    return ' قفل خروج فعال شد🔒 '
  end
end
...
local function unlock_group_leave(msg, data, target)
  ...
  if group_leave_lock == 'no' then
    return ' 🔓قفل خروج فعال نبود🔓 '
  else
    data[tostring(target)]['settings']['leave'] = 'no'
    save_data(_config.moderation.data, data)
    return ' 🔓قفل خروج غیر فعال شد🔓 '
  end
end
...
local function lock_group_operator(msg, data, target)
  ...
  if group_operator_lock == 'yes' then
    return ' 🔒قفل اوپراتور فعال بود🔒 '
  else
    data[tostring(target)]['settings']['operator'] = 'yes'
    save_data(_config.moderation.data, data)
    return ' 🔒قفل اوپراتور فعال شد🔒 '
  end
end
...
local function unlock_group_operator(msg, data, target)
  ...
  if group_operator_lock == 'no' then
    return ' 🔓قفل اوپراتور فعال نبود🔓 '
  else
    data[tostring(target)]['settings']['operator'] = 'no'
    save_data(_config.moderation.data, data)
    return ' 🔓قفل اوپراتور غیر فعال شد🔓 '
  end
end
...
local function lock_group_reply(msg, data, target)
  ...
  if group_reply_lock == 'yes' then
    return ' 🔒قفل ریپلای فعال بود🔒 '
  else
    data[tostring(target)]['settings']['reply'] = 'yes'
    save_data(_config.moderation.data, data)
    return ' 🔒قفل ریپلای فعال شد🔒 '
  end
end
...
local function unlock_group_reply(msg, data, target)
  ...
  if group_reply_lock == 'no' then
    return ' قفل ریپلای فعال نیست 🔓 '
  else
    data[tostring(target)]['settings']['reply'] = 'no'
    save_data(_config.moderation.data, data)
    return ' 🔓قفل ریپلای غیر فعال شد🔓 '
  end
end
...
local function lock_group_media(msg, data, target)
  ...
  if group_media_lock == 'yes' then
    return ' 🔒رسانه ها از قبل قفل بود🔒 '
  else
    data[tostring(target)]['settings']['media'] = 'yes'
    save_data(_config.moderation.data, data)
    return ' 🔒رسانه ها قفل شد🔒 '
  end
end
...
local function unlock_group_media(msg, data, target)
  ...
  if group_media_lock == 'no' then
    return ' 🔓قفل رسانه ها فعال نبود🔓 '
  else
    data[tostring(target)]['settings']['media'] = 'no'
    save_data(_config.moderation.data, data)
    return ' 🔓قفل رسانه ها غیر فعال شد🔓 '
  end
end
...
local function unlock_group_join(msg, data, target)
  ...
  if group_join_lock == 'no' then
    return ' 🔓قفل جوین فعال نبود🔓 '
  else
    data[tostring(target)]['settings']['join'] = 'no'
    save_data(_config.moderation.data, data)
    return ' 🔓قفل جوین غیر فعال شد🔓 '
  end
end
...
local function lock_group_english(msg, data, target)
  ...
  if group_english_lock == 'yes' then
    return ' 🔒قفل نگلیسی فعال بود🔒 '
  else
    data[tostring(target)]['settings']['english'] = 'yes'
    save_data(_config.moderation.data, data)
    return ' 🔒قفل نگلیسی فعال شد🔒 '
  end
end
...
local function unlock_group_english(msg, data, target)
  ...
  if group_english_lock == 'no' then
    return '  🔓قفل نگلیسی فعال نبود🔓 '
  else
    data[tostring(target)]['settings']['english'] = 'no'
    save_data(_config.moderation.data, data)
    return ' 🔓قفل نگلیسی غیر فعال شد🔓 '
  end
end
...
local function unlock_group_emoji(msg, data, target)
  ...
  if group_emoji_lock == 'no' then
    return ' 🔓قفل شکلک فعال نبود🔓 '
  else
    data[tostring(target)]['settings']['emoji'] = 'no'
    save_data(_config.moderation.data, data)
    return ' 🔓قفل شکلک غیر فعال شد🔓 '
  end
end
...
local function unlock_group_all(msg, data, target)
  ...
  local group_all_lock = data[tostring(target)]['settings']['all']
  if group_all_lock == 'no' then
    return ' 🔓قفل همه فعال نبود🔓 '
  else
    data[tostring(target)]['settings']['all'] = 'no'
    save_data(_config.moderation.data, data)
    return ' 🔓قفل همه غیر فعال شد🔓 '
  end
end
...
local function lock_group_flood(msg, data, target)
  ...
  if group_flood_lock == 'yes' then
    return ' 🔒قفل حساسیت فعال بود🔒 '
  else
    data[tostring(target)]['settings']['flood'] = 'yes'
    save_data(_config.moderation.data, data)
    return ' 🔒قفل حساسیت فعال شد🔒 '
  end
end
...
local function unlock_group_flood(msg, data, target)
  ...
  if group_flood_lock == 'no' then
    return ' 🔓قفل حساسیت فعال نبود🔓 '
  else
    data[tostring(target)]['settings']['flood'] = 'no'
    save_data(_config.moderation.data, data)
    return ' 🔓قفل حساسیت غیر فعال شد🔓 '
  end
end
...
local function unlock_group_rtl(msg, data, target)
  ...
  if group_rtl_lock == 'no' then
    return ' 🔓قفل آر تی ال فعال نبود🔓 '
  else
    data[tostring(target)]['settings']['lock_rtl'] = 'no'
    save_data(_config.moderation.data, data)
    return ' 🔓قفل آر تی ال غیر فعال شد🔓 '
  end
end
...
local function lock_group_tgservice(msg, data, target)
  ...
  if group_tgservice_lock == 'yes' then
    return ' 🔒قفل خروج و ورود فعال بود🔒 '
  else
    data[tostring(target)]['settings']['lock_tgservice'] = 'yes'
    save_data(_config.moderation.data, data)
    return ' 🔒قفل خروج و ورود فعال شد🔒 '
  end
end
...
local function unlock_group_tgservice(msg, data, target)
  ...
  if group_tgservice_lock == 'no' then
    return ' 🔓قفل خروج و ورود فعال نبود🔓 '
  else
    data[tostring(target)]['settings']['lock_tgservice'] = 'no'
    save_data(_config.moderation.data, data)
    return ' 🔓قفل خروج و ورود غیر فعال شد🔓 '
  end
end
...
local function lock_group_bots(msg, data, target)
  ...
  if group_bots_lock == 'yes' then
    return ' 🔒قفل بات ها فعال بود🔒 '
  else
    data[tostring(target)]['settings']['lock_bots'] = 'yes'
    save_data(_config.moderation.data, data)
    return ' 🔒قفل بات ها فعال شد🔒 '
  end
end
...
local function unlock_group_bots(msg, data, target)
  ...
  if group_bots_lock == 'no' then
    return ' 🔓قفل بات ها فعال نبود🔓 '
  else
    data[tostring(target)]['settings']['lock_bots'] = 'no'
    save_data(_config.moderation.data, data)
    return ' 🔓قفل بات ها غیر فعال شد🔓 '
  end
end
...
local function unlock_group_contacts(msg, data, target)
  ...
  if group_contacts_lock == 'no' then
    return ' 🔓قفل شماره فعال نبود🔓 '
  else
    data[tostring(target)]['settings']['lock_contacts'] = 'no'
    save_data(_config.moderation.data, data)
    return ' 🔓قفل شماره غیر فعال شد🔓 '
  end
end
...
local function disable_strict_rules(msg, data, target)
  ...
  if group_strict_lock == 'no' then
    return ' 🔓قفل سختگیرانه فعال نبود🔓 '
  else
    data[tostring(target)]['settings']['strict'] = 'no'
    save_data(_config.moderation.data, data)
    return ' 🔓قفل سختگیرانه غیر فعال شد🔓 '
  end
end
...
function show_supergroup_settingsmod(msg, target)
  ...
  if string.match(text, 'no') then text = string.gsub(text, 'no', '✖️') end
  return text
		end
	end
	...
	local data = load_data(_config.moderation.data)
		if matches[1] == 'افزودن' and not matches[2] then
			...
			if is_super_group(msg) then
				return reply_msg(msg.id, 'سوپر گروه افزوده شده بود', ok_cb, false)
			end
		...
		if matches[1] == 'حذف' and is_admin1(msg) and not matches[2] then
			if not is_super_group(msg) then
				return reply_msg(msg.id, 'سوپر گروه افزوده نشده بود', ok_cb, false)
			end
		...
		if matches[1] == "صاحب" then
			...
			if not group_owner then
				return "هیچ صاحبی در سوپر گروه وجود ندارد"
			end
			savelog(msg.to.id, name_log.." ["..msg.from.id.."] used /owner")
			return "صاحب سوپر گروه است ["..group_owner..']'
		end
		...
		if matches[1] == 'لینک جدید' and is_momod(msg)then
			...
			local receiver = get_receiver(msg)
				if success == 0 then
					send_large_msg(receiver, '*بات سازنده گپ نیست؛لطفا از دستور تنظیم لینک استفاده کنید')
		...
		if matches[1] == 'لینک' then
			...
			if not group_link then
				return "لطفا یا لینک بسازید یا از دستور تنظیم لینک استفاده کنید."
			end
		...
		if matches[1] == 'وات' and is_owner(msg) then
			local cbres_extra = {
				channelid = msg.to.id,
				get_cmd = 'res'
			}
		...
		if matches[1] == 'تنظیم صاحب' and is_owner(msg) then
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
					get_cmd = 'setowner',
					msg = msg
		...
		if matches[1] == 'تنظیم مدیر' then
		  if not is_momod(msg) then
				return
			end
			if not is_owner(msg) then
				return "فقط صاحب و ادمین میتواند"
			end
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
					get_cmd = 'promote',
		...
		if matches[1] == "تنظیم گذشته" and is_momod(msg) then
			local receiver = get_receiver(msg)
		...
		if matches[1] == 'پاک' then
			...
			if matches[2] == 'گذشته' then
				local receiver = get_receiver(msg)
				local about_text = ' '
				local data_cat = 'description'
				if data[tostring(msg.to.id)][data_cat] == nil then
					return 'گذشته ای ثبت نشده است'
				end
				data[tostring(msg.to.id)][data_cat] = nil
				save_data(_config.moderation.data, data)
		...
		if matches[1] == 'شخصی' and is_momod(msg) then
			local target = msg.to.id
			if matches[2] == 'باشه' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] set group to: public")
				return set_public_membermod(msg, data, target)
			end
			if matches[2] == 'نباشه' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: not public")
				return unset_public_membermod(msg, data, target)
			end
		end

		if matches[1] == 'خاموش' and is_owner(msg) then
			local chat_id = msg.to.id
		...
		if matches[1] == 'روشن' and is_momod(msg) then
			local chat_id = msg.to.id
			if matches[2] == 'صدا' then
			local msg_type = 'Audio'
		...
		if matches[1] == "بی صدا" or matches[1] == "صدا دار" and is_momod(msg) then
			local chat_id = msg.to.id
			local hash = "mute_user"..chat_id
			local user_id = ""
			if type(msg.reply_id) ~= "nil" then
			...
			elseif matches[1] == "بی صدا" or matches[1] == "صدا دار" and string.match(matches[2], '^%d+$') then
				local user_id = matches[2]
				if is_muted_user(chat_id, user_id) then
					unmute_user(chat_id, user_id)
			...
			elseif matches[1] == "بی صدا" or matches[1] == "صدا دار" and not string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local get_cmd = "mute_user"
				local username = matches[2]
				local username = string.gsub(matches[2], '@', '')
				resolve_username(username, callbackres, {receiver = receiver, get_ = get_cmd, msg=msg})
			end
		end

		if matches[1] == "لیست خاموش" and is_momod(msg) then
			local chat_id = msg.to.id
		...
		if matches[1] == "یست بی صدا" and is_momod(msg) then
			local chat_id = msg.to.id
